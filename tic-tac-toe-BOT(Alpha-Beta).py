...
Optimised by using minimax algorithm along with alpha beta pruning.
'''
...
def minimax(board, depth, isMax, alpha, beta, computerLetter):
	'''
	Implementation of minimax algo with alpha beta pruning.
	'''
	if computerLetter == 'X':
		playerLetter = 'O'
	else:
	...
	if isMax:
		...
		for i in range(1,10):
			...
			if isSpaceFree(copy, i):
				...
				best = max(best, minimax(copy, depth+1, not isMax, alpha, beta, computerLetter) - depth)
		...
		for i in range(1,10):
			...
			if isSpaceFree(copy, i):
				copy[i] = playerLetter
				best = min(best, minimax(copy, depth+1, not isMax, alpha, beta, computerLetter) + depth)
				beta = min(beta, best)
				if alpha >= beta:
					break
		return best
...
def getComputerMove(board, computerLetter):
	...
	for i in range(1, 10):
		...
		if isSpaceFree(copy, i):
			copy[i] = computerLetter
			moveVal = minimax(copy, 0, False, -1000, 1000, computerLetter)
			if moveVal > bestVal:
				bestMove = i
				bestVal = moveVal
